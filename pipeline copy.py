...
from pipelines_colunas.bsmt_resto import basement_pipeline

from eda import *

def preprocessamento(df):

    df = tratamento_lotfrontage(df)
    df = tratamento_alley(df)
    df = tratamento_mas_vnr_area(df)
    df = tratamento_mas_vnr_type(df)
    df = tratamento_basement(df)
    df = tratamento_electrical(df)
    df = tratamento_fireplace(df)
    df = tratamento_garage(df)
    df = tratamento_poolqc(df)
    df = tratamento_fence(df)
    df = tratamento_misc_feature(df)

    df = engenharia_features(df)

    df = transformacao_log(df)
...
def prever_precos(caminho_arquivo_teste):
    ...
    df_teste = pd.read_csv(caminho_arquivo_teste,keep_default_na=False)

    df_teste = preprocessamento(df_teste)
...
if __name__ == "__main__":
    ...
    arquivo_teste_exemplo = './data/teste_publico.csv'
